...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"4.1: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        x = self.local_conv(x)
        # print(f"4.2: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                # print(f"3.1: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
                # print("blk={}".format(blk))
                x = blk(x)
                # print(f"3.2: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        if self.downsample is not None:
            x = self.downsample(x)
        return x
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.layers[0](x)  # [8, 784, 128]
        start_i = 1
        # print(f"2.0: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        # 2.1: layers=1,torch.Size([8, 784, 128]), requires_grad=True, version=0
        # 2.2: layers=1,torch.Size([8, 196, 160]), requires_grad=True, version=0

        # 2.1: layers=2,torch.Size([8, 196, 160]), requires_grad=True, version=0
        # 2.2: layers=2,torch.Size([8, 49, 320]), requires_grad=True, version=0

        # 2.1: layers=3,torch.Size([8, 49, 320]), requires_grad=True, version=0
        # 2.2: layers=3,torch.Size([8, 49, 320]), requires_grad=True, version=0
        for i in range(start_i, len(self.layers)-1):
            layer = self.layers[i]
            # print(f"2.1: layers={i},{x.shape}, requires_grad={x.requires_grad}, version={x._version}")
            x = layer(x)
            # print(f"2.2: layers={i},{x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        # print(f"2.3: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        x = x.mean(1)


        # print(f"2.4: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        return x

    def forward(self, x):
        # print(f"1.1: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        x = self.forward_features(x)
        # print(f"1.2: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        x = self.norm_head(x)
        # print(f"1.3: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        x = self.head(x)
        # print(f"1.4: {x.shape}, requires_grad={x.requires_grad}, version={x._version}")
        return x
